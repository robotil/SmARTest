...
std::map<char,float> * SFVObject::get_Object_xyz()
{
	...
	if(! Implicit_Object_xyz->empty())
	{
		...
		float obj_x = this->get_LocationOnTheXaxis()->get_RolledValue();
		float obj_y = this->get_LocationOnTheXaxis()->get_RolledValue();
		float obj_z = 0;//map_z + this->get_LocationOnTheZaxis()->get_RolledValue();
